...
def generate_launch_description():
    ...
    default_world_path = os.path.join(test_robot_description_share, 'worlds/marker_world')
    ...
    state_broadcaster = Node(
        ...
        executable='spawner',
        arguments=['joint_state_broadcaster', 'joint_velocity_controller'],
        output='screen'
    )

    # GAZEBO_MODEL_PATH has to be correctly set for Gazebo to be able to find the model
    spawn_entity = Node(package='gazebo_ros', executable='spawn_entity.py',
                        arguments=['-entity', 'my_test_robot', '-topic', '/robot_description', '-x', '0.0', '-y', '0.0'],
                        output='screen')
